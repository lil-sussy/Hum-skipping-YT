...
function postChunkToServer({ blob, videoTime, chunkId }) {
  return new Promise((resolve, reject) => {
    ...
    const metaParts = [
      `--${boundary}`,
      `Content-Disposition: form-data; name="video_time"`,
      ``,
      String(videoTime),
    ];
    const blobHeader = [
      `--${boundary}`,
      `Content-Disposition: form-data; name="audio_blob"; filename="chunk_${chunkId}.webm"`,
  ...
  if (mediaStream && typeof MediaRecorder !== "undefined") {
    try {
      ...
      const mime = MediaRecorder.isTypeSupported("audio/webm;codecs=opus")
        ...
        : mediaStream.getAudioTracks()[0]?.contentType || "audio/webm";
